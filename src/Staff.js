...
class Staff extends PureComponent {
  render() {
    const {context, measures, height, index, width, onNoteClick, id} = this.props;
    ...
    return measures.map((measure, measureIndex) => {
      ...
      return <Measure key={measureIndex} width={measureWidth} x={x} y={y} context={context} id={measureID(id, measureIndex)} onNoteClick={onNoteClick}/>
    });
  }
}
